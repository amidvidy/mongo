...
public:
    ...
    auto then(Continuation&& continuation, ThreadPoolInterface* pool)
        -> Deferred<decltype(continuation(get()))> {
        auto state = _state;
...
template <typename U>
static Deferred<std::vector<U>> collect(std::vector<Deferred<U>>& ds, ThreadPoolInterface* pool) {
    Deferred<std::vector<U>> out;
    struct CollectState {
        // hack to avoid requiring U to be default constructible.
        std::vector<boost::optional<U>> mem{};
    ...
    for (std::size_t i = 0; i < ds.size(); ++i) {
        ds[i].template then([collectState, out, i](U res) mutable {
