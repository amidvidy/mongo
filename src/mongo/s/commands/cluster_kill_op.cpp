...
namespace {
    ...
    public:
        ...
        bool run(OperationContext* txn,
            ...
            BSONObj cmdRes;
            if (!conn->runCommand("admin", BSON("killOp" << 1 << "op" << opId ), cmdRes)) {

                using namespace mongoutils;

                // The killOp command didn't work. If it was due to the server not actually
                // having the killOp command, we try again using the old pseudo-command.
                // TODO: remove this entire block post-MongoDB 3.2
                auto status = getStatusFromCommandResult(cmdRes);

                verify(!status.isOK());

                if (status == ErrorCodes::CommandResultSchemaViolation) {
                    msgasserted(28622, str::stream() << "Received bad killOp response from shard: "
                                                     << cmdRes);
                }
                else if (status == ErrorCodes::CommandNotFound ||
                         str::startsWith(status.reason(), "no such")) {

                    conn->findOne(db + ".$cmd.sys.killop", BSON("op" << opId));
                }
            }
            conn.done();
